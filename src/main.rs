...
/*
fn is_link(p : &Path) -> Result<bool, io::Error> {
...
fn should_visit(p : &Path, fs : Option<u64>) -> bool {
	match p.metadata() {
		Err(e) => { print_error_path(p, e); false },
		Ok(meta) => {
			if meta.file_type().is_symlink() {
				...
				fs == Some(meta.as_raw_stat().st_dev)
			} else {
				true
			}
		}
...
fn visit_dirs<F : FnMut(&Path), E : FnMut(&Path, io::Error)>(dir: &Path, fs : Option<u64>, mut cb: F, mut err: E) {
	...
	if dir.is_dir() {
					...
					match entry {
						Ok(entry) => {
							...
							if should_visit(&entry_path, fs) {
								visit_dirs(&entry_path, fs, |p| cb(p), |p, e| err(p, e));
							} else {
								cb(&entry_path);
							}
						},
		...
		err(dir, io::Error::new(io::ErrorKind::InvalidInput, dir.to_str().unwrap_or("")));
	}
}
...
fn visit_dirs_summary(dir: &Path, config : &CmdConfig) {
	...
	let fs = if config.localfs {
		match dir.metadata() {
			...
			Ok(stat) => Some(stat.as_raw_stat().st_dev)
		}
	} else { None };
				...
				match entry {
					Ok(entry) => {
						...
						let size = if should_visit(&entry_path, fs) {
							let mut size = 0;
							visit_dirs(&entry_path, fs, |p| if p.is_file() {
								match p.metadata() {
							...
							match entry.metadata() {
								Err(e) => { print_error_path(&entry_path, e); 0}
								Ok(meta) => { meta.len() }
							}
						};
						entries.insert(SizeSortedFile{entry: entry_path.to_path_buf(), size: size});
					},
